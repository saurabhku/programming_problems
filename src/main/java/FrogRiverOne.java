 ...
 *
 * The goal is to find the earliest time when the frog
...
public class FrogRiverOne {
    public int solution(int X, int[] A) {
        ...
        int[] earliestPosition = new int[X];
        int sum =0;
        if (X%2 == 0) {
            sum = (X /2)*(2+ (X-1));
        } else
        ...
        for (int count =0; count<X; count++ ) {
            earliestPosition[count]  = -1;
        }
        for (int count =0; count<A.length; count++) {
            int element = A[count];
            if (earliestPosition[element-1] == -1 ) {
                earliestPosition[element-1] = count;
                sum = sum- A[count];
                if (sum == 0 ) {
                    result = count;
                    break;
