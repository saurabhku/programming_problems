 ...
 * in different places. The height of the wall is specified by an array H of N
 * positive integers. H[I] is the height of the wall from I to I+1 meters to the
 * ight of its left end. In particular, H[0] is the height of the wall's left end
...
public class StoneWall {
    int solution(int H[]) {
        ...
        for(int count =0; count< H.length; count++ ) {
            ...
            if (stack.isEmpty()) {
                stack.push(H[count]);
            } else if (H[count] > stack.peek()) {
                stack.push(H[count]);
            } else if (H[count] < stack.peek()) {
                while (!stack.isEmpty() && (stack.peek() > H[count]) ) {
                    stack.pop();
                    boxCount++;
                }
                if (stack.isEmpty())  {
