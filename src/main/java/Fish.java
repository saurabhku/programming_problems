...
public class Fish {
    public int solution(int[] A, int[] B) {
        ...
        Stack<Integer> fishesGoingUpstream = new Stack<>();
        Stack<Integer> fishesGoingDownStream = new Stack<>();

        int numberOfAliveFishes = 0;
        for (int count = 0; count < B.length; count++) {
            if (B[count] == 0) {
                while (!fishesGoingDownStream.isEmpty() && A[fishesGoingDownStream.peek()] < A[count]) {
            ...
            if (B[count] == 1) {
                fishesGoingDownStream.push(count);
            }

        }
        if (!fishesGoingDownStream.isEmpty()) numberOfAliveFishes = numberOfAliveFishes+fishesGoingDownStream.size();
